...
#' @export
gearPlotHHN21.nodp<-function(Survey="SP-NORTH",years=2021,quarter=4,c.inta=.8,c.intb=.3,col1="darkblue",col2="red",incl2=TRUE,getICES=TRUE,pF=TRUE,ti=TRUE) {
  if (getICES) {
      ...
      plot(Netopening~Depth,dumb,xlim=c(0,dpthA[2]+20),ylim=c(0,vrt[2]+2),type="n",pch=21,col=col1,
         ylab="Vertical opening (m)",xlab="Depth (m)",subset=Year!=years[length(years)] & Netopening> c(-9))
          ...
          if (length(unique(dumb$Ship))==2) {
           ...
           if (pF) {
              points(Netopening~Depth,dumbmol,subset=HaulVal=="V",pch=21,col=col2)   
              points(Netopening~Depth,dumbmol,subset=Year==years[length(years)],pch=21,bg=col2,lwd=1)   
              points(Netopening~Depth,dumbvde,subset=HaulVal=="V",pch=21,col=col1)   
              points(Netopening~Depth,dumbvde,subset=Year==years[length(years)],pch=21,bg=col1,lwd=1)   
