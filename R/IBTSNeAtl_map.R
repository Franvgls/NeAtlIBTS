...
IBTSNeAtl_map<-function(nl=60.5,sl=36.0,xlims=c(-18,3),leg=TRUE,legpos=c("bottomright"),cex.leg=.7,dens=30,
  ...
  ices.areas_wg84<-sf::st_transform(ices.areas, crs = 32630)
  ...
  if (bathy) {
    sp::plot(bath100[1],add=T,col=ifelse(bw,gray(.85),gray(.50)),lwd=.1)
    plot(bathy.geb[bathy.geb$DEPTH!=100,1],add=T,col=ifelse(bw,gray(.85),gray(.50)),lwd=.1)
  }
  # if (grid) grid(col=gray(.8),lwd=.5)
  if (ICESdiv) sp::plot(ices.div[1],add=T,col=NA,border="burlywood")
  if (all(xlims < 0)) {
  ...
  if (load){
    #maps::map(SWC_Q1_w84,SWC_Q1_w84$Name,add=T,col="yellow",lwd=.1,fill=T,dens=dens,angle=0)
    #sp::plot(SWC_Q1_w84,add=T,col="yellow",lwd=.01,dens=dens,angle=0)
    sf::plot(SWC_Q1[1],add=T,col="yellow",dens=dens)
