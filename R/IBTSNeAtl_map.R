...
IBTSNeAtl_map<-function(nl=60.5,sl=36,xlims=c(-18,3),leg=TRUE,cex.leg=.7,dens=30,ICESdiv=TRUE,icesLab=TRUE,bathy=TRUE,out="tiff",nfile="NeAtlIBTS_map",lwdl=.1,shpdir="c:/users/francisco.velasco.ST/documents/fvg/campañas/IBTS/shapes/") {
  ...
  if (ICESdiv & !exists("ices.div")) ices.div<-readOGR(paste0(shpdir,"ices_div.dbf"),"ices_div")
  if (bathy & !exists("bath100")) {
    bath100<-readOGR(paste0(shpdir,"100m.dbf"),"100m")
    bathy.geb<-readOGR(paste0(shpdir,"bathy_geb.dbf"),"bathy_geb")
  }
  if (!exists("SWC_Q1")) {
    SWC_Q1<-readOGR(paste0(shpdir,"SWC_Q1.dbf"),"SWC_Q1")
    SWC_Q1_w84<-spTransform(SWC_Q1,CRS("+proj=longlat +datum=WGS84"))}
  if (!exists("SWC_Q3")) {
    SWC_Q3<-readOGR(paste0(shpdir,"SWC_Q3.dbf"),"SWC_Q3")
    SWC_Q3_w84<-spTransform(SWC_Q3,CRS("+proj=longlat +datum=WGS84"))}
  if (!exists("IGFS")) {
    IGFS<-readOGR(paste0(shpdir,"IGFS.dbf"),"IGFS")
    IGFS_w84<-spTransform(IGFS,CRS("+proj=longlat +datum=WGS84")) }
  if (!exists("NIGFS")) {
    NIGFS<-readOGR(paste0(shpdir,"NI_IBTS.dbf"),"NI_IBTS")
    NIGFS_w84<-spTransform(NIGFS,CRS("+proj=longlat +datum=WGS84")) }
  if (!exists("CGFS")) CGFS<-readOGR(paste0(shpdir,"CGFS_stratum.dbf"),"CGFS_stratum")
  if (!exists("Porc")) {
    Porc<-readOGR(paste0(shpdir,"Porcupine.dbf"),"Porcupine")
    Porc_w84<-spTransform(Porc,CRS("+proj=longlat +datum=WGS84")) 
  }
  if (!exists("EVHOE")) {
    EVHOE<-readOGR(paste0(shpdir,"EVHOE.dbf"),"EVHOE")
    EVHOE_w84<-spTransform(EVHOE,CRS("+proj=longlat +datum=WGS84"))
    }
  if (!exists("Sp_North")) {
    Sp_North<-readOGR(paste0(shpdir,"Sp_North.dbf"),"Sp_North")
    Sp_North_w84<-spTransform(Sp_North,CRS("+proj=longlat +datum=WGS84")) 
    }
  if (!exists("Sp_Cadiz")) {
    Sp_Cadiz<-readOGR(paste0(shpdir,"Sp_Cadiz.dbf"),"Sp_Cadiz")
    Sp_Cadiz_w84<-spTransform(Sp_Cadiz,CRS("+proj=longlat +datum=WGS84"))
    }
  if (!exists("PT_IBTS")) PT_IBTS<-readOGR(paste0(shpdir,"PT_IBTS_2015.dbf"),"PT_IBTS_2015")
  switch(out,
...
#  windows()
  ...
  box()
  map(SWC_Q1_w84,SWC_Q1_w84$Name,add=T,col=gray(.4),lwd=.1,fill=T,dens=dens,angle=0)
  map(SWC_Q3_w84,SWC_Q3_w84$Name,add=T,col=2,lwd=.1,fill=T,dens=dens,angle=45)
  map(NIGFS_w84,NIGFS_w84$NAME,add=T,col=3,lwd=.1,fill=T,dens=dens,angle=90)
